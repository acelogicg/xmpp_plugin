...
public class FlutterXmppPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case pluginMethod.retractMessage:
            self.performRetractMessage(call, result)

        case pluginMethod.enablePush:
            self.performEnablePush(call, result)
        ...
        case pluginMethod.readMessage:
            self.performReadMessageActivity(call, result)
    ...
    func performRetractMessage(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let vData = call.arguments as? [String: Any] else {
            result(xmppConstants.DataNil)
            return
        }
        let toJid = (vData["to_jid"] as? String ?? "").trim()
        let targetId = (vData["target_id"] as? String ?? "").trim()
        let retractId = (vData["id"] as? String ?? "").trim()
        if toJid.isEmpty || targetId.isEmpty || retractId.isEmpty {
            result(xmppConstants.DataNil)
            return
        }
        self.objXMPP.sendMessageRetraction(
            toJid: toJid,
            targetId: targetId,
            retractId: retractId,
            withStrem: self.objXMPP.xmppStream
    ...
    func performEnablePush(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let vData = call.arguments as? [String: Any] else {
            result(xmppConstants.DataNil)
            return
        }
        let pushJid = (vData["push_jid"] as? String ?? "").trim()
        let node = (vData["node"] as? String ?? "").trim()
        let appId = (vData["app_id"] as? String ?? "").trim()
        if pushJid.isEmpty {
            result(xmppConstants.DataNil)
            return
        }
        self.objXMPP.enablePush(pushJid: pushJid, node: node, appId: appId, withStrem: self.objXMPP.xmppStream)
        result(xmppConstants.SUCCESS)
    }
    ...
    func performReadMessageActivity(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        guard let vData = call.arguments as? [String : Any] else {
            result(xmppConstants.ERROR)
            return
        }
        let vMethod : String = call.method.trim()
        let toJid : String = (vData["to_jid"] as? String ?? "").trim()
        let msgId : String = (vData["id"] as? String ?? "").trim()
        printLog("\(#function) | \(vMethod) | arguments: \(vData) | toJid: \(toJid) | msgId : \(msgId)")

        self.objXMPP.sendReadReceipt(jid: toJid, messageId: msgId, withStrem: self.objXMPP.xmppStream)
        result(xmppConstants.SUCCESS)
    }
