...
extension XMPPController {
    ...
    func sendReadReceipt(jid: String, messageId: String, withStrem: XMPPStream) {
        if jid.trim().isEmpty {
            print("\(#function) | jid is empty/nil.")
            return
        }
        ...
        let eleRead: XMLElement = XMLElement.init(name: eleRead.Name, xmlns: eleRead.Namespace)
        eleRead.addAttribute(withName: eleRead.Key, stringValue: messageId)
        xmppMessage.addChild(eleRead)
    ...
    func sendAckReadReceipt(withMessageId : String) {
        let vMessId = withMessageId.trim()
        let vFrom : String = ""
        let vBody : String = ""
        let dicDate = ["type" : pluginMessType.ACK_READ,
                       "id" : vMessId,
                       "from" : vFrom,
                       "body" : vBody,
                       "msgtype" : "normal"]
...
//        }
    ...
    func sendMessageRetraction(toJid: String, targetId: String, retractId: String, withStrem: XMPPStream) {
        if toJid.trim().isEmpty || targetId.trim().isEmpty || retractId.trim().isEmpty {
            print("\(#function) | invalid args.")
            return
        }
        guard let jid = XMPPJID(string: toJid) else { return }
        let xmppMessage = XMPPMessage(type: xmppChatType.CHAT, to: jid)
        xmppMessage.addAttribute(withName: "id", stringValue: retractId)
        let applyTo = XMLElement(name: "apply-to", xmlns: "urn:xmpp:message-retract:0")
        applyTo.addAttribute(withName: "id", stringValue: targetId)
        let retract = XMLElement(name: "retract", xmlns: "urn:xmpp:message-retract:0")
        applyTo.addChild(retract)
        xmppMessage.addChild(applyTo)
        withStrem.send(xmppMessage)
        addLogger(.sentMessageToServer, xmppMessage)
    }

    func enablePush(pushJid: String, node: String, appId: String, withStrem: XMPPStream) {
        if pushJid.trim().isEmpty {
            print("\(#function) | pushJid is empty.")
            return
        }
        guard let jid = XMPPJID(string: pushJid) else { return }

        let enable = XMLElement(name: "enable", xmlns: "urn:xmpp:push:0")
        enable.addAttribute(withName: "jid", stringValue: pushJid)
        if !node.trim().isEmpty {
            enable.addAttribute(withName: "node", stringValue: node.trim())
        }

        let x = XMLElement(name: "x", xmlns: "jabber:x:data")
        x.addAttribute(withName: "type", stringValue: "submit")

        let fieldForm = XMLElement(name: "field")
        fieldForm.addAttribute(withName: "var", stringValue: "FORM_TYPE")
        fieldForm.addAttribute(withName: "type", stringValue: "hidden")
        fieldForm.addChild(XMLElement(name: "value", stringValue: "urn:xmpp:push:0"))

        let fieldApp = XMLElement(name: "field")
        fieldApp.addAttribute(withName: "var", stringValue: "appid")
        fieldApp.addChild(XMLElement(name: "value", stringValue: appId))

        x.addChild(fieldForm)
        x.addChild(fieldApp)
        enable.addChild(x)

        let iq = XMPPIQ(type: "set", to: jid, elementID: XMPPStream.generateUUID(), child: enable)
        withStrem.send(iq)
        addLogger(.sentMessageToServer, iq)
    }
