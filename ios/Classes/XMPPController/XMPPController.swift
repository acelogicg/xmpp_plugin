...
extension XMPPMessage {
    ...
    func getReadReceiptId() -> String {
        let arrRead = self.elements(forName: eleRead.Name)
        guard let ele = arrRead.first else {
            return ""
        }
        let vInfo = ele.attribute(forName: eleRead.Key)?.stringValue ?? ""
        return vInfo.trim()
    }
