...
extension XMPPController {
    ...
    func handel_ChatMessage(_ message: XMPPMessage, withType type : String, withStrem : XMPPStream) {
        ...
        if let applyTo = message.element(forName: "apply-to", xmlns: "urn:xmpp:message-retract:0"),
           let targetId = applyTo.attributeStringValue(forName: "id") {
            let dicDate = ["type" : "retract",
                           "id" : vId,
    ...
    func handelNormalChatMessage(_ message: XMPPMessage, withStrem : XMPPStream) {
        let readId = message.getReadReceiptId().trim()
        if !readId.isEmpty {
            self.sendAckReadReceipt(withMessageId: readId)
            return
        }
