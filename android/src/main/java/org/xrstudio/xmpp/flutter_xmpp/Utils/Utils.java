...
//        checkDirectoryExist(logFilePath);
    ...
    public static void broadcastMessageToFlutter(Context mApplicationContext, Message message) {
        ...
        StandardExtensionElement retractElement =
                (StandardExtensionElement) message.getExtension("apply-to", "urn:xmpp:message-retract:0");
        if (retractElement != null) {
            META_TEXT = "retract";
            String targetId = retractElement.getAttributeValue("id");
            if (targetId != null) customText = targetId;
        }
        ...
        ExtensionElement readExt = message.getExtension(Constants.READ, Constants.URN_XMPP_READ);
        if (readExt instanceof StandardExtensionElement) {
            String readId = ((StandardExtensionElement) readExt).getAttributeValue(Constants.ID);
        ...
        if (message.hasExtension(Constants.URN_XMPP_DELAY)) {
        ...
        // Bundle up the intent and send the broadcast (include self messages too).
