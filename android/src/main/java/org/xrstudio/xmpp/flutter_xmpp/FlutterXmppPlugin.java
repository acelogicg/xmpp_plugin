// android/src/main/java/org/xrstudio/xmpp/flutter_xmpp/FlutterXmppPlugin.java
...
public class FlutterXmppPlugin implements MethodCallHandler, FlutterPlugin, ActivityAware, EventChannel.StreamHandler {
    ...
    public static void sendRetraction(String toUser, String targetId, String retractId) {
        FlutterXmppConnection.sendMessageRetraction(toUser, targetId, retractId);
    }
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case Constants.RETRACT_MESSAGE: {
                ...
                String retractId = call.argument(Constants.ID);
                if (toJid == null || targetId == null || retractId == null) {
                    result.error("MISSING", "Missing argument to_jid / target_id / id.", null);
                    return;
                }
                sendRetraction(toJid, targetId, retractId);
                result.success(Constants.SUCCESS);
                break;
            }
            ...
            case Constants.ENABLE_PUSH: {
                ...
                if (pushJid == null || pushJid.isEmpty()) {
                    result.error("MISSING", "Missing argument push_jid.", null);
                    return;
                }
            ...
            case Constants.GET_MAM: {
                String userJidArg = call.argument(Constants.userJid);
                ...
                Utils.printLog("userJId " + userJidArg + " Before : " + requestBefore + " since " + requestSince + " limit " + limit);
                try {
                    MAMManager.requestMAM(userJidArg, requestBefore, requestSince, limit);
                    result.success("SUCCESS");
                } catch (Exception e) {
                    result.error("MAM_ERROR", e.getMessage(), null);
                }
                break;
            }
